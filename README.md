...
> 🎨 **2026 改版**：浅色**冷色调医疗风**（医疗蓝 + 青绿）+ 苹果式滚动动效 + 全新 Logo 的前端；导航升级为页面内真实路线 + 转向步骤 + 一键跳转地图 App；医院推荐升级为「症状 → 分诊 → **按对症专科检索** → 按匹配度排序 + 推荐理由 + 真实评价」；**全局 12 语言**（首屏选语言、全站即时切换）。
...
| 模块 | 说明 |
|------|------|
| 🗣️ **多语种翻译** | 多语互译 + **2400+ 医学术语**（ICD-10 专业语料）自动对齐，置信度评分 + 4 级风险提示 |
...
- **浅色冷色调医疗前端**：医疗蓝(`#1A78C2`) + 青绿(`#0E9488`)点缀 + 冷灰底 + 衬线大标题 + 苹果式滚动入场/视差/毛玻璃 + 全新十字/对话气泡 Logo；桌面 / 平板 / 手机自适应，支持 `prefers-reduced-motion`
- **全局多语言（12 种，即时）**：首屏选语言→全站切换。英文 / 中文手写精翻；其余 10 种由平台自身 `/api/translate` 引擎在**构建期一次性预生成**（`gen_i18n.py` → `i18n_all.json`），内嵌进 `app.js`，运行期**零延迟切换**（不再运行时联网翻译）。导航转向步骤用**离线模板**即时翻成所选语言（保留路名中文；地图 POI 标注仍为中文）
